...
function Signup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { firstName, password, confirmPassword, email } = data;
    if (firstName && email && password && confirmPassword) {
      if (password === confirmPassword) {
        ...
        alert("confirm apswd and paswd not matching");
      }
    } else {
      ...
      <div className="w-full max-w-sm bg-white m-auto flex  flex-col p-2">
        ...
        <form className="w-full py-3 flex flex-col" onSubmit={handleSubmit}>
          ...
          <input
            ...
            id="firstName"
            name="firstName"
            className="mt-1 w-full  bg-slate-200 px-2 py-1 mb-2 rounded focus-within:outline-blue-300"
